...
- **Activity chart time ranges** — pick between 1D (hourly buckets), 3H (15-minute buckets), and a realtime 15M tab (1-minute buckets) from the chart-header tabs, the single control; tab switches persist as the default range and legacy preferences migrate. The 15M tab refreshes once a minute while the popover is open and stops the moment it closes; fine-grained tabs tighten the popover-open refresh throttle to 5 minutes, and quiet windows render as zero lines instead of empty states. The underlying indexes now keep a 1-minute bucket base (rebuilt once on upgrade) that every granularity derives from, with 1-minute responses trimmed to the trailing 15 minutes and cached for 60 seconds.
